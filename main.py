...
def generateToken():
    ...
    global regReq
    try:
        system(f"title Token Generator V3ㅣ{round(generatedTokens / ((time() - genStartTime) / 60))}/mㅣSuccess {generatedTokens}ㅣFailed {failedTokens}ㅣError {erroredTokens} ㅣ Solved {solved}")
                    ...
                    if captchaData.get("status") == "ready":
                        ...
                        if emailData.status_code == 400:
                            ...
                            if emailData.status_code == 400:
                                ...
                                print(f"Succesfully passed captcha on retry!")
                                pass
